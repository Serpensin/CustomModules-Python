...
class BaseDatabaseBackend(ABC):
    ...
    async def execute(
        ...
        fetch: Optional[
            Union[str, bool]
        ] = None,  # Can be 'one', 'all', False (for rowcount), or None
...
class SQLiteBackend(BaseDatabaseBackend):
    ...
    async def connect(self, connection_params: Dict[str, Any]) -> None:
        ...
        # Enable WAL mode for better concurrent performance - batch pragmas for efficiency
        pragmas = [
...
class MySQLBackend(BaseDatabaseBackend):
    ...
    async def connect(self, connection_params: Dict[str, Any]) -> None:
        ...
        self.logger.debug(
            f"MySQL connection pool established: {connection_params.get('database')} "
            f"(pool size: {self.pool_minsize}-{self.pool_maxsize})"
        )
    ...
    def _convert_query_cached(self, query: str) -> str:
        """Cache query conversions for performance"""
        # Convert SQLite ? placeholders to MySQL %s placeholders
        converted_query = query.replace("?", "%s")

        # Convert SQLite AUTOINCREMENT to MySQL AUTO_INCREMENT
        converted_query = self._AUTOINCREMENT_PATTERN.sub(
            "AUTO_INCREMENT", converted_query
        )

        # Convert CURRENT_TIMESTAMP default syntax if needed
        converted_query = self._CURRENT_TIMESTAMP_PATTERN.sub(
            "DEFAULT CURRENT_TIMESTAMP", converted_query
        )
...
class PostgreSQLBackend(BaseDatabaseBackend):
    ...
    async def connect(self, connection_params: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"PostgreSQL (asyncpg) connection pool established: {connection_params.get('database')} "
                f"(pool size: {self.pool_minsize}-{self.pool_maxsize})"
            )
        except ImportError:
            ...
            self.logger.debug(
                f"PostgreSQL (psycopg) connection pool established: {connection_params.get('database')} "
                f"(pool size: {self.pool_minsize}-{self.pool_maxsize})"
            )
    ...
    def _convert_query_cached(self, query: str) -> str:
        ...
        converted_query = self._AUTOINCREMENT_PATTERN.sub(
            "SERIAL PRIMARY KEY", converted_query
        )
    ...
    def _normalize_params(
        ...
        """Normalize parameters to tuple format"""
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                try:
                    ...
                    self.logger.error(
                        f"PostgreSQL (asyncpg) query execution error: {e}"
                    )
                    raise
        ...
        async with self.pool.connection() as conn:
            async with conn.cursor() as cursor:
                try:
                    ...
                    await conn.rollback()
                    self.logger.error(
                        f"PostgreSQL (psycopg) query execution error: {e}"
                    )
                    raise
            ...
            async with self.pool.acquire() as conn:
                async with conn.transaction():
                    try:
                        ...
                        self.logger.debug(
                            f"Committed batch transaction: {len(params_list)} rows affected"
                        )
                        return len(params_list)
                    except Exception as e:
                        self.logger.error(
                            f"PostgreSQL (asyncpg) batch query execution error: {e}"
                        )
                        raise
        else:  # psycopg
            async with self.pool.connection() as conn:
                async with conn.cursor() as cursor:
                    try:
                        ...
                        await conn.rollback()
                        self.logger.error(
                            f"PostgreSQL (psycopg) batch query execution error: {e}"
                        )
                        raise
...
class AsyncDatabaseHandler:
    ...
    def __init__(
        self, backend: BaseDatabaseBackend, logger: Optional[logging.Logger] = None
    ):
    ...
    async def create(
        ...
        Args:
            ...
            pool_minsize: Minimum pool size (for pooled backends)
            pool_maxsize: Maximum pool size (for pooled backends)
